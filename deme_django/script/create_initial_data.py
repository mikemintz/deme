...
admin.save_versioned(agent=None, first_agent=True, create_permissions=False)
...
default_layout.save_versioned(agent=admin)
...
home_page.save_versioned(agent=admin)
...
mike_person.save_versioned(agent=admin)
...
mike_authentication_method.save_versioned(agent=mike_person)
WebauthAuthenticationMethod(username="mintz", agent=mike_person).save_versioned(agent=mike_person)
mike_email_contact_method = EmailContactMethod(name="Mike's Email Contact Method", email="mike@example.com", agent=mike_person)
mike_email_contact_method.save_versioned(agent=mike_person)

todd_person = Person(first_name="Todd", last_name="Davies", name="Todd Davies")
todd_person.save_versioned(agent=admin)
...
{% endblock content %}
""")
hello_page.save_versioned(agent=admin)
...
hello_url.save_versioned(agent=admin)

symsys_group = Group(name="Symsys Group")
symsys_group.save_versioned(agent=admin)
Membership(item=mike_person, collection=symsys_group).save_versioned(agent=admin)
Membership(item=todd_person, collection=symsys_group).save_versioned(agent=admin)
Membership(item=reid_person, collection=symsys_group).save_versioned(agent=admin)

discuss_group = Group(name="Deme Dev Discussion")
discuss_group.save_versioned(agent=admin)
Membership(item=mike_person, collection=discuss_group).save_versioned(agent=admin)
Membership(item=todd_person, collection=discuss_group).save_versioned(agent=admin)
Subscription(contact_method=mike_email_contact_method, item=discuss_group.folios.get(), deep=True, notify_text=True, notify_edit=True).save_versioned(agent=mike_person)
Subscription(contact_method=todd_email_contact_method, item=discuss_group.folios.get(), deep=True, notify_text=True, notify_edit=True).save_versioned(agent=todd_person)
