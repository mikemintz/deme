$(function(){
  $('#metadata_content_related_items').on('contentload', function(){
    ...
    content_section.find('.type-related-item-add').each(function(){
      ...
      // add button
      var newbtn = $('<a href="#" class="newbtn btn btn-info btn-sm" title="New Item"><i class="glyphicon glyphicon-plus"></i></a>').appendTo(action_wrap);
      newbtn.click(function(e){
        e.preventDefault();
        var random_num = Math.floor((Math.random()*1000000)+1);
        window.open(new_modal_url, 'embedform-' + random_num, 'width=600,toolbar=1,location=yes,resizable=1,scrollbars=yes,height=600,top=100,left=100');
      });
    });
  });
});